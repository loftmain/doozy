    ...
    def modeling(self):
        ...
        model.fit(
            train_x, train_y, validation_data=(
                test_x, test_y), epochs=self._epo, verbose=False)
...
for i, v in enumerate(feature_list): feature_list[i] = v.replace('.csv','')

features = list(combinations(feature_list, 3))
...
for feature in features:
    ...
    mod.set_option(feature, target_list, train_size)
    # 옵션 설정
    # 경제지표 목록, 레이블 타겟, 트레이닝 사이즈
    ...
    mod.merge_data(data, path, 'rate', 3)
    # 지수데이터와 경제지표 데이터가 모두 들어있는 데이터셋(데이터프레임)을 만듬
    # 지수데이터 데이터프레임, 경제지표가 들어있는 폴더 경로
    # 경제지표에서 어떤 정규화 데이터를 쓸것인지
    # data shifting value는 얼마인지
    ...
    f = open('log.txt', 'a+')
    f.write(str(feature)+(str(scoring))+'\n')
    f.close()
    
    if np.array(scoring).mean() > 0.6:
        res.to_csv('model/RNN'+str(scoring)+'.csv', header=True, index=False)
        # csv파일 생성