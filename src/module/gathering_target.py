...
class Gathering_target:
    ...
    20200818 189 LINE -> monthly 데이터 받아오는 파라미터 추가
    20200819 209 LINE -> FDR 기능 사용 X(monthly data 가공,처리 로직 오류있음)
    ㅡㅡㅡ
    ...
    def gathering(self, path):
        ...
        if self._opt == 'PDR':
            ...
            try:
                df = web.get_data_yahoo(self._code, self._spoint, interval='m')
            except:
                print(self._code,
                      ': 이름이 잘못되었거나 접속이 지연되고 있습니다')
                pass

            pdf = self.preprocessing(df)
...
if __name__ == "__main__":
    ...
    folder_name = 'dependent'
    # 만들어질 폴더 이름

    starting_point = '2008-01-01'
    # 지수데이터 시작 날짜
